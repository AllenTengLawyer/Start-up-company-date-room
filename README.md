...
- **Word 报告 (.docx)** — LDD 就绪度报告，支持中/英文切换
- **HTML 报告** — 浏览器查看，支持打印，支持中/英文切换
- **文件夹复制** — 按分类结构复制到指定目录
- **JSON 备份/恢复** — 完整项目数据备份（含分类、文件、LDD 状态、创始人）
- **LDD 压缩包** — 按尽调章节/条目整理文件 + LDD_Notes.txt 说明
- **分类压缩包** — 导出某一分类及其子分类的全部文件
- **创始人报告** — 创始人背景核查 Word 报告 + 所有附件 ZIP
...
- **全文搜索**: SQLite FTS5
- **导出**: python-docx (Word), Jinja2 (HTML模板)
...
- **导出面板** — Word 报告、HTML 报告、文件夹复制、创始人报告

注意：
- **Word 导出**依赖 python-docx，中文字体使用 Microsoft YaHei，Windows 环境下显示最佳。
- **文件夹导出**建议选择有写权限的目标目录，路径长度不超过 240 字符（Windows 限制）。
